...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!symbol) {
    console.error("❌ No symbol found in URL.");
    return;
  }

  console.log("Loaded symbol:", symbol);
  loadCandlestick(symbol);
...
function resizeHandler() {
  ...
  resizeTimeout = setTimeout(() => {
    const urlParams = new URLSearchParams(window.location.search);
    const stockId = urlParams.get("symbol");
    if (symbol) {
...
function loadCandlestick(symbol) {
  fetch(`/api/candle?symbol=${symbol}`)
    .then((res) => {
      if (!res.ok)
